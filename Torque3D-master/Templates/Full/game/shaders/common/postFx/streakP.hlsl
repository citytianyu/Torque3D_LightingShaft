...
float4 sample_offsets[NUM_SAMPLES];
float4 sample_weights[NUM_SAMPLES];  // coloured!
...
float4 main(in float2 uv : TEXCOORD0) : COLOR
{
  ...
  for (int i=0; i < NUM_SAMPLES; i++)	{
    sample += float4(sample_weights[i].xyz,1) * tex2D(src_texture, uv + sample_offsets[i].xy);
  }
